...
export class UserEntity {
    ...
    @Column({ type: 'varchar', length: 50, enum: UserRole, default: UserRole.USER })
    role: UserRole; // Rol del usuario (por ejemplo, 'admin', 'user', etc.)
    
    @Column({ type: 'varchar', length: 50, enum: UserStatus, default: UserStatus.ACTIVE })
    ...
    getDisplayName(): string {
        return this.name.toLowerCase().split(' ').map((word) => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
